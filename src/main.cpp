...
void updateAlarmData(const String& key, int value) {
  if (key == "year") {
    fbYear = value;
  } else if (key == "month") {
    fbMonth = value;
...
void streamCallback(FirebaseStream data) {
  ...
  if (data.dataTypeEnum() == fb_esp_rtdb_data_type_json) {
    ...
    // Verificamos e atualizamos cada campo
    if (json->get(result, "year")) updateAlarmData("year", result.intValue);
    if (json->get(result, "month")) updateAlarmData("month", result.intValue);
    if (json->get(result, "day")) updateAlarmData("day", result.intValue);
    if (json->get(result, "hour")) updateAlarmData("hour", result.intValue);
    if (json->get(result, "minute")) updateAlarmData("minute", result.intValue);
    if (json->get(result, "second")) updateAlarmData("second", result.intValue);
    ...
    digitalWrite(debugLedPin, val ? HIGH : LOW);
    ...
    updateAlarmData(subKey, val);
    
    // Agora que os dados estão atualizados, agendamos o alarme
    scheduleAlarm(fbYear, fbMonth, fbDay, fbHour, fbMinute, fbSecond);
  }
